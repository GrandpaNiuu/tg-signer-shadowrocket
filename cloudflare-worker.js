/**
 * Cloudflare Worker: trigger GitHub Actions workflow_dispatch for tg-signer.
 *
 * Required Worker variables:
 * - TRIGGER_KEY: shared key used by Shadowrocket trigger.js
 * - GITHUB_TOKEN: fine-grained GitHub token with Actions: write permission
 * - GITHUB_OWNER: repository owner, for example GrandpaNiuu
 * - GITHUB_REPO: repository name, for example tg-signer-shadowrocket
 * - GITHUB_WORKFLOW_FILE: workflow file name, default daily-checkin.yml
 * - GITHUB_REF: branch name, default main
 */
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname !== "/run") {
      return json({ ok: false, error: "Use /run to trigger checkin." }, 404);
    }
