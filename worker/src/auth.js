...
function normalizeTeamDomain(value) {
  if (typeof value !== "string" || !value) throw new Error("Access team domain is missing.");
...
export async function verifyAdminRequest(request, env, dependencies = {}) {
  ...
  if (env.ACCESS_ALLOW_HEADER === "true" && headerEmail) {
    if (expectedEmail && headerEmail !== expectedEmail) {
      throw new HttpError(403, "admin_forbidden", "This identity is not the configured administrator.");
    }
    return { email: headerEmail, source: "trusted_header" };
  }

  try {
    if (!env.ACCESS_AUD) throw new Error("Access audience is missing.");
    ...
    if (!email) throw new Error("Access email is missing.");
    if (expectedEmail && email !== expectedEmail) {
      throw new HttpError(403, "admin_forbidden", "This identity is not the configured administrator.");
    }
    return { email, source: "access_jwt" };
  } catch (error) {
    if (error instanceof HttpError) throw error;
    throw new HttpError(401, "admin_authentication_required", "Cloudflare Access authentication is required.");
  }
}
