...
test("Telegram Skill expansion adds canonical params, run snapshots, media assets, and Skill rows", () => {
  ...
  assert.deepEqual(
    sqlite.prepare("SELECT skill_key FROM skills WHERE skill_key IN ('bot_flow','send_media','chat_snapshot','account_audit') ORDER BY skill_key")
      .all().map((row) => row.skill_key),
    ["account_audit", "bot_flow", "chat_snapshot", "send_media"],
  );
